...
export const SET_PUBLICATIONS = '@@objectsearch/SET_PUBLICATIONS';
export const SET_STATUS = '@@objectsearch/SET_STATUS';
export const SET_ERROR = '@@objectsearch/SET_ERROR';

const setPublications = (value) => ({
...
export const getPublications = (token, searchData) => async (dispatch) => {
    ...
    try {
        ...
        if (response.ok) {
            const { items } = json;
            const ids = [];
            items.forEach(({ encodedId }) => {
                ids.push(encodedId);
            });
            dispatch(setPublications(ids));
            dispatch(getDocuments(token, ids));
